...
//then Alice takes 11 points. On the other hand, if Alice shot 0 arrows on the section with score 11 and Bob shot 2 arrows on that same section,
...
class MaximumPointsInAnArcheryCompetition{
   int finalSum = 0;
    ...
    public int[] maximumBobPoints(int numArrows, int[] aliceArrows) {
        ...
        maximumBobPoints(numArrows, aliceArrows, 0, 0, myArr);
        return arr;
    }
    
    public void maximumBobPoints(int numArrows, int[]aliceArrows, int idx, int sum, int[]myArr){
        ...
        int alice = aliceArrows[idx];
        if(numArrows-alice-1>=0){
            myArr[idx] = alice+1;
            maximumBobPoints(numArrows-alice-1, aliceArrows, idx+1, sum+idx, myArr);
            myArr[idx] = 0;
        }
        
        maximumBobPoints(numArrows, aliceArrows, idx+1, sum, myArr);
