...
public class AllAncestorsOfANodeInADirectedAcyclicGraph {
    public List<List<Integer>> getAncestors(int n, int[][] edges) {
        ...
        TreeSet[] connections = new TreeSet[n];
        ...
        while(que.size()!=0){
            ...
            while(size-->0){
                ...
                TreeSet<Integer> rmSet = connections[rm];
                ArrayList<Integer> rmEdges = graph[rm];
                for(Integer e : rmEdges){
                    indegree[e]--;
                    connections[e].add(rm);
                    for(Integer c : rmSet)
                        connections[e].add(c);
                    if(indegree[e]==0)
                        que.addLast(e);
        ...
        for(int i=0;i<n;i++){
            List<Integer> smallAns = new ArrayList<>(connections[i]);
            ans.add(smallAns);
        }
