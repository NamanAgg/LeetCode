...
public class ProductOfArrayExceptSelf {
    public int[] productExceptSelf(int[] nums) {
        ...
        leftProduct[nums.length]=1;
        for(int i=nums.length-1;i>=0;i--)            //for making leftProduct
            leftProduct[i]=nums[i]*leftProduct[i+1];
        
        rightProduct[0]=1;
        for(int i=0;i<nums.length;i++)               //for making rightProduct
            rightProduct[i+1]=nums[i]*rightProduct[i];
        
        int[]ans=new int[nums.length];
        
        for(int i=0;i<nums.length;i++)                  //this is main logic
            ans[i] = rightProduct[i] * leftProduct[i+1];
