...
public class DistinctSubsequencesII {
  ...
  public int distinctSub_memo(String s, int n, int[] dp, int[] freq) {
    ...
    if (freq[s.charAt(n - 1) - 'a'] == 0) { //we haven't seen this char
      ...
      additonalSum =
        (
          prevRes -
          distinctSub_memo(s, freq[s.charAt(n - 1) - 'a'] - 1, dp, freq) +
          mod
        ) %
        mod;
    }
  ...
  public int distinctSub_DP(String s, int N, int[] dp, int[] freq) {
    for (int n = 0; n <= N; n++) {
      if (n == 0) {
        dp[n] = 0;
        continue;
      ...
      int prevRes = dp[n - 1];
      int additonalSum = 0;
      if (freq[s.charAt(n - 1) - 'a'] == 0) { 
        additonalSum = (1 + prevRes) % mod;
      } 
      else {
        additonalSum =
          (prevRes - dp[freq[s.charAt(n - 1) - 'a'] - 1] + mod) % mod;
      }
      freq[s.charAt(n - 1) - 'a'] = n;
      dp[n] = (prevRes + additonalSum) % mod;
    }
