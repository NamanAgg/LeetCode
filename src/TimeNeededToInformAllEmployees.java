...
public class TimeNeededToInformAllEmployees {
    public int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
		...
		// start bfs from headID
        Queue<Integer> q=new LinkedList<Integer>();
        q.add(headID);
        int timeArray[]=new int[informTime.length]; 
        int time=0; 
        while(!q.isEmpty()) {
        	int person=q.remove();
        	for(int sub:list.get(person)) {
				int t=informTime[person]+timeArray[person];
        		timeArray[sub]+=t;
        		q.add(sub);
        		time=Math.max(time, timeArray[sub]);
        	}
        }
        return time; 
    }
